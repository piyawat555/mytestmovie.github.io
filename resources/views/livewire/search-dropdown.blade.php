<div class="relative mt-3 md:mt-0" x-data="{ isOpen: true }" @click.away="isOpen = false">
    <input
        ...
        type="text"
        class="bg-gray-800 text-sm rounded-full w-64 px-4 pl-8 py-1 focus:outline-none focus:shadow-outline" placeholder="ค้นหาภาพยนต์"
        x-ref="search"
        @keydown.window="
    ...
    @if (strlen($search) >= 2)
        ...
        >
            ...
            @else
                <div class="px-3 py-3">ไม่พบภาพยนต์เรื่องนี้ "{{ $search }}"</div>
            @endif
        </div>
    @endif
</div>
