    ...
    <div class="movie-info border-b border-gray-800">
        <div class="container mx auto px-4 py-16 flex flex-col md:flex-row">
            ...
            <img src="{{$actor['profile_path']}}" alt="profile image" class="w-76">
                    <ul class="flex items-center mt-4">
                        @if ($social['facebook'])
                        <li>
                        <a href="{{$social['facebook']}}" title="Fackbook">
           ...
           <div class="md:ml-24">
              <h2 class="text-4xl font-semibold">ชื่อนักแสดง : {{$actor['name']}}</h2>
                ...
                <span class="ml-2">{{$actor['birthday']}} (อายุ {{$actor['age']}} ปี) เกิดใน {{$actor['place_of_birth']}}</span>
                   ...
                   <p class="text-gray-300 mt-8">
                    ...
                    <h4 class="font-semibold mt-12">ภาพยนต์ที่เคยเล่น</h4>

                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8">
                        @foreach($KnowforMovie as $casts)
                        <div class="mt-4">
                            <a href="{{$casts['linkToPage']}}">
                            <img src="{{$casts['poster_path']}}" alt="poster">
                            </a>
                        <a href="{{$casts['linkToPage']}}" class="text-sm leading-normal block text-gray-400 hover:text-white mt-1">{{$casts['title']}}</a>
                        </div>
                        @endforeach
                    </div>
            <div>
        ...
        <div class="credits border-b border-gray-800">
            <div class="container mx-auto px-4 py-16">
                <h2 class="text-4xl font-semibold">ประวัติการเล่นภาพยนตร์/ซีรีย์</h2>
                <ul class="list-disc leading-loose pl-5 mt-8">
                    @foreach ($credits as $item)
                <li> {{$item['release_year']}} &middot; <strong>{{$item['title']}}</a> as {{$item['character']}}</strong></li>
                    @endforeach
                </ul>
            </div>
        </div>
